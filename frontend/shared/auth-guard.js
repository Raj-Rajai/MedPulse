...
(function() {
  ...
  var isPublicPage = PUBLIC_PATHS.indexOf(currentPath) !== -1 || currentPath === '/login' || currentPath === '/register';
  var isAdminPage = currentPath === '/admin.html' || currentPath === '/admin' || currentPath === '/admin/' || currentPath.indexOf('/admin/') === 0;
  var isPatientPage = currentPath === '/patient.html' || currentPath === '/patient' || currentPath === '/patient/' || currentPath.indexOf('/patient/') === 0;
  var isHospitalPage = currentPath === '/hospital.html' || currentPath === '/hospital' || currentPath === '/hospital/' || currentPath.indexOf('/hospital/') === 0;
  ...
  if (typeof window.fetch === 'function') {
    ...
    window.fetch = function(input, init) {
      ...
      if (activeAdmin && (url.indexOf('/api/admin/') !== -1 || isAdminPage)) {
        ...
        if (!headers.has('Authorization')) {
          headers.set('Authorization', 'Bearer admin-' + activeAdmin.id);
        }
        init.headers = headers;
      }
      // Inject Hospital Admin headers for /api/hospital/ calls
      else if (activeHospitalAdmin && (url.indexOf('/api/hospital/') !== -1 || isHospitalPage)) {
      ...
      else if (activePatient && (url.indexOf('/api/patient/') !== -1 || isPatientPage)) {
        if (!headers.has('X-Patient-Id') && activePatient.id) {
          headers.set('X-Patient-Id', String(activePatient.id));
        }
        if (!headers.has('Authorization')) {
          headers.set('Authorization', 'Bearer patient-' + activePatient.id);
        }
        init.headers = headers;
      }
      ...
      return originalFetch.call(this, input, init).then(function(response) {
        ...
        if (response.status === 401 && (url.indexOf('/api/admin/') !== -1 || isAdminPage) && url.indexOf('/api/admin/login') === -1) {
        ...
        else if (response.status === 401 && (url.indexOf('/api/hospital/') !== -1 || isHospitalPage) && url.indexOf('/api/hospital/login') === -1) {
        ...
        else if (response.status === 401 && (url.indexOf('/api/patient/') !== -1 || isPatientPage) && url.indexOf('/api/patient/login') === -1 && url.indexOf('/api/patient/register') === -1) {
        ...
        else if (response.status === 401 && !isPublicPage && !isAdminPage && !isPatientPage && !isHospitalPage && url.indexOf('/api/auth/login') === -1 && url.indexOf('/api/auth/register') === -1) {
  ...
  window.renderSidebarUserBadge = renderSidebarUserBadge;
